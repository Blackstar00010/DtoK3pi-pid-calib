...
int main() {
    std::string figDir = config::plot_dir5(commonVars::modelName, commonVars::cut, "comp");
    auto [file, tree] = loadSweight();

    std::vector<std::string> particles = {"K", "pi1", "pi2", "pi3"};
    std::vector<std::string> modes = {"fine", "PIDCalib2"};
    for (const auto& particle : particles) {
        for (const auto& mode : modes) {
            ...
            std::vector<std::vector<double>> myData = generateSigYieldData(
                tree, {particle + "_P", particle + "_ETA"}, commonVars::weightCol, 
                {rowBins, colBins}, {{rowMin, rowMax}, {colMin, colMax}}
            );
            ...
            Plot2dHist(
                ...
                "p [MeV/c]", "\\eta", {{rowMin, rowMax}, {colMin, colMax}}, {min, max});
